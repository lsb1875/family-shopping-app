...
with st.container(border=True):
    ...
    new_item = st.text_input(
        "무엇을 살까요?", placeholder="재료 입력...", key="input_new_item"
    )
...
if not shopping_list:
    ...
    for i, full_item in enumerate(shopping_list):
        user, name = (
            full_item.split(":", 1) if ":" in full_item else ("기본", full_item)
        )
    ...
    if not st.session_state.confirm_delete:
        if st.button(
            "🧹 전체 목록 삭제", use_container_width=True, key="clear_all_btn"
        ):
        ...
        with st.container(border=True):
            ...
            if st.button(
                ...
                st.rerun()
            if st.button(
                "❌ 아니오, 취소합니다", use_container_width=True, key="confirm_no"
            ):
                st.session_state.confirm_delete = False
                st.rerun()
